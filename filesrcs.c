    ...
    while ((c = fgetc(f)) != EOF) {
        ...
        if (all_utf8) {
            ...
            if (!is_valid_utf8_sequence(f, c, &bytes_read)) {
                ...
                // Consume continuation bytes to advance file pointer
                for (int i = 1; i < bytes_read; i++) {
                    fgetc(f); // Already read in is_valid_utf8_sequence, so consume here
                }
            }
        }
    }
...
enum file_type detect_filetype(const char *path) {
    ...
    if (is_ascii) {
        return ASCII; // kun ASCII tegn
    }

    return DATA; // 2.1.2 foreløbig fake Print one line with the path and a "fake type"
